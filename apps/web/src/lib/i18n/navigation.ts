 ...
 * which this codebase never imports, because `src/proxy.ts` owns routing and
 * must stay the single authority on what a URL means.
 *
 ...
 * so the proxy stays redirect-only and the evaluated path is always the path
 * the router serves. `as-needed` would need a rewrite, and then those two
 * strings differ — a standing invitation to routing/auth mismatches.
 */
