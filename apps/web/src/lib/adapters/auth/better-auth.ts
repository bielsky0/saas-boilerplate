...
export const auth = betterAuth({
  ...
  plugins: [
    ...
    // MCP / OAuth lives in Nest since faza 2.7 (`apps/api`, `McpController`);
    // the web keeps only the `/oauth/login` + `/oauth/consent` UI bridge and
    // proxies (`/api/mcp`, `/.well-known/*`, `/api/auth/*`), so the plugin is
    // gone from this engine — one authorization server, not two.
