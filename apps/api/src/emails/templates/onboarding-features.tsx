 ...
 *
 * Never reaches anyone who already subscribed: the handler re-checks that at
 * run time, so the upgrade nudge cannot land on a paying customer.
 */
export function OnboardingFeatures(
  ...
  return (
    <EmailLayout preview={t("onboarding-features.preview")}>
      ...
      <Button href={`${links.appUrl}/dashboard`}>{t("onboarding-features.cta")}</Button>
