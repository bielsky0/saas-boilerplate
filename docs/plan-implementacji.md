...
- **⚠️ Redirect z Server Action omija proxy (ujawnione w F4.6, dotyczy każdej przyszłej akcji przekierowującej):** `redirect()` w Server Action jest rozwiązywany przez Next **wewnętrznie** — cel renderuje się w tym samym cyklu, bez nowego żądania — więc `src/proxy.ts` go nie widzi i nie dokłada ani prefiksu locale, ani `x-org-subdomain`. Objaw był odległy od przyczyny: po zalogowaniu na host akademii renderował się panel **konta osobistego**, a odświeżenie naprawiało wszystko. Reguła na przyszłość: **akcja przekierowująca nie może emitować gołej ścieżki** i nie może zakładać, że warstwa żądania dostanie kontekst z proxy. `servedSubdomain()` ma od F4.6 fallback na `parseHost(Host)` (D73), ale pierwszy render po takim redirectcie nadal nie rozstrzyga tenanta — pozostałość odnotowana w raporcie F4.6, do domknięcia w fazie dotykającej tej warstwy.
