...
| **Mechanika Stripe dla rabatu klienta na subskrypcji** (v15, spec §8 pkt 8): `coupon`/`promotion_code` na Subscription, cena ad-hoc per klient, czy aktualizacja `subscription_item` przed odnowieniem. `stripe_price_id` to stała cena i nie wyrazi rabatu zmiennego między cyklami (Rozstrzygnięcie #17 spec) | **start F21** — blokuje tę fazę; nie blokuje pozostałego zakresu                 |
