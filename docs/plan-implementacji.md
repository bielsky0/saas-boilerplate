...
| **Routing subdomenowy** (rewizja 15.1, §2.27): rozpoznanie tenanta z nagłówka `Host` w `src/proxy.ts` — obsługujące **zarówno trasy aplikacji (`/dashboard`, `/admin`), jak i publiczne strony CMS** (rozgałęzienie po prefiksie ścieżki: zarezerwowana → routing aplikacji, reszta → `page` po slugu przez Payload), nie tylko publiczną rejestrację. Częścią tego samego zadania jest **jedno źródło listy zarezerwowanych sluggów** (`src/features/cms/reserved-slugs.ts`), importowane przez middleware i przez walidację formularza tworzenia strony. Dodatkowo: domeny własne przez CNAME, wildcard DNS/TLS, zachowanie na `localhost` w dev i e2e. Kolumna `organization.subdomain` istnieje od F0; brakuje samego middleware. Migracja tras `/orgs/[slug]/…` → `/dashboard/…` należy do tego zadania (patrz „Dług routingu z Fazy 2" w Ryzykach) | **zależność blokująca przed EPIK 4** (publiczna rejestracja) — nie blokuje F1–F3. **Blokuje też start modułu CMS (Website Builder, `docs/specyfikacja-cms.md`)** — jedna zależność wspólna dla obu modułów, nie dwa równoległe routingi |
...
- **Dług routingu z Fazy 2 (potwierdzony 2026-07-20):** UI CRUD lokalizacji/typów grup/wzorców/grafiku żyje pod `/orgs/[slug]/…`. Przy implementacji middleware'u subdomenowego (przed EPIK 4 / modułem CMS) przenieść te trasy pod `{organization.subdomain}.langlion.pl/dashboard/…` — sama struktura folderów tras, bez zmian w RBAC/warstwie danych/jobach, które są routing-agnostyczne. Faza 2 pozostaje zamknięta i zrealizowana zgodnie z modelem obowiązującym w momencie jej implementacji; do czasu zbudowania middleware'u `/orgs/[slug]/…` jest jedynym działającym mechanizmem i nie należy go przepisywać wcześniej.
- **Dług adresowania z Fazy 3 (2026-07-20):** trasy `/api/client-auth/*` przyjmują `organization.subdomain` jako pole żądania (D39). Po wdrożeniu middleware'u subdomenowego wartość ma pochodzić z nagłówka `Host`, a pole zostać usunięte z kontraktu — zmiana dotyczy **wyłącznie miejsca pozyskania tego samego stringa** (`findOrganizationBySubdomain` i wszystko za nim jest bez zmian). Świadomie wybrano subdomenę zamiast `organizationId` właśnie po to, żeby ta migracja była usunięciem pola, a nie przeprojektowaniem tras.
